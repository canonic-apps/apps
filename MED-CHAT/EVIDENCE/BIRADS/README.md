# BIRADS — Evidence Source

inherits: /MED-CHAT/EVIDENCE/

---

## Purpose

BIRADS = Breast Imaging Reporting and Data System.

**Authority:** GOLD (imaging classification standard)

---

## Categories

| Category | Assessment | Cancer Risk |
|----------|------------|-------------|
| 0 | Incomplete | Needs additional imaging |
| 1 | Negative | ~0% |
| 2 | Benign | ~0% |
| 3 | Probably Benign | <2% |
| 4 | Suspicious | 2-95% |
| 5 | Highly Suggestive | >95% |
| 6 | Known Biopsy-Proven | 100% |

---

## Current Version

```
ACR BI-RADS Atlas, 5th Edition
Published: 2013
Authority: American College of Radiology
```

---

## Context Awareness

BIRADS category MUST be interpreted with:
- Patient age
- Prior imaging history
- Clinical presentation
- Risk factors

---

## Citation Format

```
BIRADS.5ed.Category.X
```

---

*BIRADS | GOLD evidence | Imaging classification*
